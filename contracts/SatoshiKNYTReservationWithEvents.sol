...
/**
 * @title 21 Sats Reservation Contract with Referral Events
 * @dev Enhanced version with blockchain event tracking for referrals
 * 
...
contract SatoshiKNYTReservation is ERC721Enumerable, Ownable {
    ...
    // NEW: Event for tracking referral mints on blockchain
    event ReferralMint(
    ...
    // ENHANCED: Now properly handles referrer and emits events
    function mintWithReferral(uint256 quantity, address referrer) external payable {
    ...
    function _mintFullWithReferrer(uint256 quantity, address referrer) internal {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            // ENHANCED: Emit referral event if referrer provided
            if (referrer != address(0)) {
                emit ReferralMint(
                    msg.sender,
                    referrer,
                ...
                // Update referral tracking (for existing functionality)
    ...
    function mintReferralShard(address to, uint256 quantity) external onlyOwner {
        require(quantity > 0, "Quantity must be positive");
        require(referralShardsMinted + quantity <= REFERRAL_SHARD_SUPPLY, "Exceeds referral shard supply");
    ...
    // NEW: Get total supply for compatibility
