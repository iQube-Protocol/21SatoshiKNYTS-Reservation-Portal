...
contract SatoshiKNYTReservation is ERC721Enumerable, Ownable {
    ...
    // Absolutely minimal referral function - identical to mintFull but accepts referrer
    function mintWithReferral(uint256 quantity, address referrer) external payable {
        // Just call the internal function exactly like mintFull does, ignoring referrer for now
    ...
    function mintFullWithReferrer(uint256 quantity, address referrer) external payable {
        // Forward to the working function
        this.mintWithReferral{value: msg.value}(quantity, referrer);
    }
    ...
    function _mintFullWithReferrer(uint256 quantity, address referrer) internal {
        ...
        // Calculate starting token ID for this transaction
        uint256 currentSupply = totalSupply();
        uint256 nextTokenId = currentSupply + 1;
        
        // Process referral reward if valid referrer provided
        // Temporarily simplify to isolate the exact issue
        if (referrer != address(0) && referrer != msg.sender && referralShardsMinted < REFERRAL_SHARD_SUPPLY) {
            // Just increment the counter for now - no actual minting
            referralShardsMinted += 1;
            nextTokenId++; // Still increment to maintain token ID consistency
        }
        
        // Mint full tokens to user
        for (uint256 i = 0; i < quantity; i++) {
            _passTypes[nextTokenId] = PassType.Full;
